        ...
        for bias in (DataType.NONE, DataType.BF16):
            for s1_shape, s2_shape in shapes:
                if s2_shape != Stage2TileShape.M32_N256_K256 and (
                    ...
                    continue
                for p1 in (WeightLoadPolicy.NON_TEMPORAL,):
                    for p2 in (WeightLoadPolicy.NON_TEMPORAL,):
                        solution = MoeSolutionId(
                            hidden=hidden,
...
def _get_2stage_cfgs_cached(token, requested, arch, policy1, policy2, explicit, is_ep, route_reduce):
    ...
    if explicit is not None:
        ...
        if kimi:
            preferred1 = WeightLoadPolicy.NON_TEMPORAL
            preferred2 = WeightLoadPolicy.NON_TEMPORAL
            ...
            preferred1 = preferred2 = WeightLoadPolicy.NON_TEMPORAL
