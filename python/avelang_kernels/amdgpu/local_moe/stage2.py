...
K256 stages input chunks through LDS and uses weight-as-A MFMA. K128 keeps
input resident, uses activation-as-A MFMA and interleaves the final compute
cluster with BF16 stores. Local wrappers own task selection, routing and global
output writes; MegaMoE supplies its own scheduling and output publication.
"""
...
def _make_weighted_bf16_pair(bias):
    ...
    def pack_weighted_pair(
        ...
        values[0, 1] = second
        if BIAS:
            values[0, 0] = values[0, 0] + al.convert(bias_first, al.f32)
            values[0, 1] = values[0, 1] + al.convert(bias_second, al.f32)
        packed = al.make_local((1, 2), al.bf16)
        values[0, 0] = values[0, 0] * route_weight
        values[0, 1] = values[0, 1] * route_weight
        packed[0] = al.convert(values[0], al.bf16)
        # Two BF16 values occupy one u32; a one-element view is scalar.
        packed_word = al.view(packed, al.u32, al.make_layout((1,), (1,)))
        return packed_word

    return pack_weighted_pair


@cache
def make_stage2_compute_k256(intermediate, bias, weight_cache, act_cache=0, words=STAGE2_K256_LDS_WORDS):
    """Write weighted BF16 [32, 256] output into LDS and synchronize."""
    I, BIAS, K_TILES = intermediate, bias, intermediate // 256
    pack_weighted_pair = _make_weighted_bf16_pair(bias)
    ...
    def stage2_compute_k256(
        ...
        shared = al.view(storage, al.u32, al.make_layout((32, 128), (128, 1)))
            ...
            if row < work_m:
                for n in al.static_range(4):
                    for pair in al.static_range(2):
                        bias_first, bias_second = al.convert(0, al.bf16), al.convert(0, al.bf16)
                        if BIAS:
                            bias_first = bias_values[n, pair * 2]
                            bias_second = bias_values[n, pair * 2 + 1]
                        pairs[pair] = pack_weighted_pair(
                            accum[n, m, pair * 2],
                            accum[n, m, pair * 2 + 1],
                            bias_first,
                            bias_second,
                    ...
                    col = wave * 32 + n * 8 + (lane // 16) * 2
                    shared[row, col] = pairs[0]
                    shared[row, col + 1] = pairs[1]
        al.syncthreads()
...
@cache
def _make_activation_a_fragment_store(tile_m, words):
    BM, MR, WORDS = tile_m, tile_m // 16, words

    @avelang.jit
    def write_fragment(
        accum: al.Tensor((MR, 4), al.f32),
        route_weights: al.Tensor((MR, 4), al.f32),
        storage: al.Tensor((WORDS,), al.u32),
        wave: al.u32,
        lane: al.u32,
        n: al.u32,
    ):
        values = al.view(accum, al.f32, al.make_layout((MR, 2, 2), (4, 2, 1)))
        weights = al.view(route_weights, al.f32, al.make_layout((MR, 2, 2), (4, 2, 1)))
        shared = al.view(storage, al.bf16, al.make_layout((BM, 256), (256, 1)))
        packed = al.make_local((1, 2), al.bf16)
        for m in al.static_range(MR):
            for pair in al.static_range(2):
                # Keep the multiply and BF16 conversion as float2 operations.
                # The two halves belong to adjacent rows in the C-shuffle.
                packed[0] = al.convert(values[m, pair] * weights[m, pair], al.bf16)
                row = m * 16 + (lane // 16) * 4 + pair * 2
                col = wave * 64 + n * 16 + lane % 16
                shared[row, col] = packed[0, 0]
                shared[row + 1, col] = packed[0, 1]
...
def make_stage2_compute_k128(intermediate, tile_m, scale_columns, weight_cache):
    """Reuse resident-input LDS for weighted BF16 [tile_m, 256] output.
    ...
    write_fragment = _make_activation_a_fragment_store(BM, WORDS)
    ...
    def stage2_compute_k128(
        ...
        route_weight_bits = al.make_local((MR, 4), al.u32)
        epilogue_weights = al.view(route_weight_bits, al.f32, al.make_layout((MR, 4), (4, 1)))
        ...
        for k in al.static_range(KT):
            ...
            if k + 1 == KT:
                for m in al.static_range(MR):
                    offset = al.convert((m * 4 + lane // 16) * 16, al.u32)
                    route_weight_bits[m] = al.amdgpu.raw_buffer_load_x4(rw_resource, offset, 0, 0)
                al.syncthreads()
            al.amdgpu.sched_barrier(0)
            al.amdgpu.s_setprio(1)
            if k + 1 == KT:
                # Store the preceding N fragment while the next one computes.
                for n in al.static_range(4):
                    for m in al.static_range(MR):
                        accum[n, m] = al.amdgpu.mfma_scale_16x16x128_fp4(
                            fragments[m],
                    ...
                    if n > 0:
                        write_fragment(accum[n - 1], epilogue_weights, storage, wave, lane, al.convert(n - 1, al.u32))
                write_fragment(accum[3], epilogue_weights, storage, wave, lane, al.convert(3, al.u32))
            else:
    ...
    def stage2(
        ...
        for block in al.range(begin, begin + assigned):
            ...
            if expert < E:
                ...
                shared = al.view(storage, al.bf16, al.make_layout((32, 128, 2), (256, 2, 1)))
                ...
                for m in al.static_range(8):
                    ...
                    if row_offset < output_bytes:
                        ...
                        al.amdgpu.raw_buffer_atomic_add_bf16x2(shared[row, lane], output_resource, offset)
                        al.amdgpu.raw_buffer_atomic_add_bf16x2(shared[row, lane + 64], output_resource, offset + 256)
...
def _make_stage2_kernel_k128(config: MoeConfig):
    D, I, E, TOPK = config.hidden, config.intermediate, config.experts, config.topk
    BM, MR = config.stage2_tile_m, config.stage2_tile_m // 16
    ...
    def stage2(
        ...
        shared = al.view(storage, al.bf16, al.make_layout((BM, 128, 2), (256, 2, 1)))
        output_routes = al.make_local((BM // 8,), al.u32)
        for group in al.static_range(MR):
            for half in al.static_range(2):
                row = (group * 2 + half) * 8 + tid // 32
                output_routes[group * 2 + half] = al.amdgpu.raw_buffer_load_x1(route_resource, row * 4, 0, 0)
        al.syncthreads()
        for group in al.static_range(MR):
            for half in al.static_range(2):
                row = (group * 2 + half) * 8 + tid // 32
                packed_token = output_routes[group * 2 + half]
                    ...
                    for n in al.static_range(4):
                        ...
                        if ROUTE_OUTPUT:
                            ...
                            al.amdgpu.raw_buffer_atomic_add_bf16x2(
                                shared[row, pair_col], output_resource, offset + al.convert(n * 128, al.u32)
                            )
