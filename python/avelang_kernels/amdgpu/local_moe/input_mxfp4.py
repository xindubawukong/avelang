...
def make_mxfp4_input(config):
    ...
    def prefetch_input(
        ...
        al.amdgpu.raw_buffer_load_x4_lds(act, storage, 16, offset, 0, (k % 2 * STRIDE + wave * 8 * 32) * 4, 0)
        if wave == 0:
            offset_s = (block * (D // 256) + k) * 256 + lane * 4
            al.amdgpu.raw_buffer_load_x1_lds(scales, storage, 4, offset_s, 0, (k % 2 * STRIDE + ACT_WORDS) * 4, 0)
    ...
    def read_input(
        ...
        return storage[k % 2 * STRIDE + ACT_WORDS + lane]
