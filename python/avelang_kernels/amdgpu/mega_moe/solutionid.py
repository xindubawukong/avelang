"""Petit MegaMoE's compact ID, with topology encoded in bits 24 through 49."""
...
class MegaMoeSolutionId:
    ...
    def __post_init__(self):
        ...
        if self.world_size not in (1, 2, 4, 8):
            raise ValueError("world_size must be 1, 2, 4 or 8")
        if not 32 <= self.experts <= 512 or self.experts % 32 or self.experts % self.world_size:
            raise ValueError("experts must be a multiple of 32 in [32, 512], divisible by world_size")
        if not 1 <= self.topk <= min(15, self.experts):
            raise ValueError("topk must be in [1, min(15, experts)]")
