...
_PROFILES = (
    *((r, 32, 4, 2880, 3072, ActivationFunction.OPENAI_SWIGLU, DataType.BF16, (0,)) for r in (1, 2, 4, 8)),
...
def get_2stage_cfgs(tokens, world_size, experts, topk, hidden, intermediate, *, activation, bias_dtype):
    ...
    if isinstance(activation, str):
        try:
            activation = {"silu": ActivationFunction.SILU_DOT, "swiglu": ActivationFunction.OPENAI_SWIGLU}[activation]
        except KeyError:
            raise ValueError("activation must be 'silu' or 'swiglu'") from None
...
def make_kernels(config):
    ...
    if config.solution.world_size == 1:
        from .api import make_single_rank_kernel

        return (make_single_rank_kernel(config),)
