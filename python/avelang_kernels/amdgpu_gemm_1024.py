...
@avelang.jit
def _staggered_k_tile(group_n: al.u32, k_total: al.u32, offset: al.u32) -> al.u32:
    k_start = (group_n & STAGGER_U_MASK) * STAGGER_U_STRIDE
...
def _gemm_pipeline_transposed_b_kernel(
    ...
    _load_global_a(a_rsrc, k, group_m, _staggered_k_tile(group_n, k_total, 0), tid, reg_a)
    _load_global_b(b_rsrc, k, group_n, _staggered_k_tile(group_n, k_total, 0), tid, reg_b)
    ...
    _load_global_a(a_rsrc, k, group_m, _staggered_k_tile(group_n, k_total, 1), tid, reg_a)
    _load_global_b(b_rsrc, k, group_n, _staggered_k_tile(group_n, k_total, 1), tid, reg_b)
    ...
    for k_idx in al.range(0, k_total - 3, 2):
        ...
        _store_shm_a(shm_a, reg_a, tid)
        _store_shm_b(shm_b, reg_b, tid)
        _load_global_a(a_rsrc, k, group_m, _staggered_k_tile(group_n, k_total, k_idx + 2), tid, reg_a)
        _load_global_b(b_rsrc, k, group_n, _staggered_k_tile(group_n, k_total, k_idx + 2), tid, reg_b)
        al.syncthreads()
        ...
        _store_shm_a(shm_a, reg_a, tid)
        _store_shm_b(shm_b, reg_b, tid)
        _load_global_a(a_rsrc, k, group_m, _staggered_k_tile(group_n, k_total, k_idx + 3), tid, reg_a)
        _load_global_b(b_rsrc, k, group_n, _staggered_k_tile(group_n, k_total, k_idx + 3), tid, reg_b)
        al.syncthreads()
...
def gemm_pipeline_transposed_b(
    ...
    if (m, n, k) != (1024, 1024, 1024):
        raise ValueError(
            f"amdgpu_gemm_1024 only supports M=N=K=1024, got M={m}, N={n}, K={k}."
        )
    if m % GROUP_M != 0:
