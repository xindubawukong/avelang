...
| K128 Stage2 | `stage2.py` | Full I resident in compact swizzled LDS, weight as MFMA A, double weight registers, K256 scale reuse across K128 tiles, interleaved last-tile C-shuffle with packed 64-bit XOR-swizzled stores and route-ID prefetch |
| K256 Stage2 | `stage2.py` | Direct native scale loads, weights before input LDS write, fewer barriers, packed valid-row C-shuffle with the same XOR output mapping as K128, per-wave writeback |
...
- Stage2's private epilogue helpers live next to their respective pipelines.
  K256 retains accumulator traversal and bias application in its pipeline.
  Both paths reuse a weighted BF16 pair conversion that takes two scalar
  accumulator values, keeping the helper independent of the complete tile.
  K128 calls its fragment writer between the final MFMA clusters; four adjacent
  BF16 columns are combined into one 64-bit LDS store. Its XOR row swizzle is
  applied again when reading pairs for atomic or per-route output.
  `output_word_index` defines this mapping once for both Local pipelines and
  MegaMoE's route-output writer; its column argument is in BF16 elements and
  its result is a u32 word index.
...
Both Stage2 pipelines use weight as MFMA operand A. K256 remains separate because it
stages each input chunk through LDS, while K128 retains all input tiles and
interleaves the last compute cluster with its output stores.
...
extending the bound's lifetime across the GEMM.

The Stage2 C-shuffle converts float2 values to packed BF16 pairs with
shape-preserving vector `al.convert`. K256 writes the pairs to LDS; K128
combines two pairs into a 64-bit store.
